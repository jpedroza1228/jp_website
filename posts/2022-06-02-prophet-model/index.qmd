...
For the sake of not waiting for this to render, I decided to make a RDS file of the metrics gathered from the tuned Prophet model. We can see that the RMSE value was `r metrics$mean[[1]]` and the prior scale changepoint value was `r metrics$prior_scale_changepoints[[1]]`, the prior scale seasonality value was `r metrics$prior_scale_seasonality[[1]]`, and the prior scale holiday value was `r metrics$prior_scale_holidays[[1]]`. 
...
We can see that when using the whole training set, we have a RMSE of `r final_train$rmse` and a MASE of `r final_train$mase` so both metrics reduced slightly.
...
Well, that doesn't look very good and we can see that with the metrics. The MASE has gotten much worse (`r test_model$mase`) and so has the RMSE (`r test_model$rmse`) ![sweating](https://media.giphy.com/media/32mC2kXYWCsg0/giphy.gif) 
